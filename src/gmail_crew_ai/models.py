...
class SimpleCategorizedEmailList(BaseModel):
    """Wrapper model for a list of categorized emails."""
    emails: List[SimpleCategorizedEmail] = Field(..., description="List of categorized emails")
class OrganizedEmailList(BaseModel):
    """Wrapper model for a list of organized emails."""
...
class SlackNotificationList(BaseModel):
    """Wrapper model for a list of Slack notifications."""
    notifications: List[SlackNotification] = Field(..., description="List of Slack notifications")

class EmailCleanupReport(BaseModel):
    """Wrapper model for cleanup report."""
    deleted_emails: List[EmailCleanupInfo] = Field(default_factory=list, description="List of deleted emails")
    preserved_emails: List[EmailCleanupInfo] = Field(default_factory=list, description="List of preserved emails")
    trash_status: Dict[str, Any] = Field(default_factory=dict, description="Trash emptying status")
